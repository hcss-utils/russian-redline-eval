...
def main(app_path):
    ...
    if mm:
        ...
        for mo in models:
            ...
            if not src:
                # a MODELS entry with no decision records is not "nothing to check", it is a
                # configuration the page claims to have run and the data does not know about
                fails.append(f"MODELS[{mo.get('k')}] has no decision records")
                continue
            C = cm[src]
        ...
        # the page's slate and the data's slate must be the same set, both directions
        page_keys = {mo.get("k") for mo in models}
        data_keys = {k.replace("-", "_").replace(".", "_") for k in scores["models"]}
        if page_keys != data_keys:
            fails.append(f"MODELS slate differs from scores.json; "
                         f"page-only {sorted(page_keys-data_keys)}, data-only {sorted(data_keys-page_keys)}")
    ...
    if mm:
        ...
        for mo in models:
            ...
            if not src:
                fails.append(f"MODELS[{mo.get('k')}] is absent from scores.json")
                continue
    ...
    # --- 3d. the Method class table ----------------------------------------
    # It showed 66 / 16 / 28: the first two from the 100-item sample, the third from the
    # 298-item reference set. Two populations in one column, under one header.
    if samp_p:
        want_cls = {"None": 0, "RLS": 0, "NTS": 0}
        for r in samp.values():
            want_cls["NTS" if r["gold_nts"] == "Y" else ("RLS" if r["gold_rls"] == "Y" else "None")] += 1
        mt = re.search(r"In the 100.*?</thead>(.*?)</table>", app, re.S)
        if not mt:
            fails.append("Method class table not found (header 'In the 100' absent)")
        else:
            cells = [int(x) for x in re.findall(r"<td>(\d+)</td>", mt.group(1))][:3]
            want = [want_cls["None"], want_cls["RLS"], want_cls["NTS"]]
            if cells != want:
                fails.append(f"Method class table shows {cells}, the 100-item sample gives {want}")
    ...
    # --- 6. no legacy data consumer survives anywhere -----------------------
    # A rename that chases one variable name leaves the others: a.fabr-b.fabr in a sort
    # comparator survived, comparing undefined to undefined, and the bars rendered in
    # arbitrary order with no error. Reject the PATTERN, not the name.
    # CSS selectors (.ev-chip.fab) and palette keys are not data reads; everything else is.
    NOT_DATA = ("chip", "VZ")
    legacy = [m_.group(0) for m_ in re.finditer(r"\b[A-Za-z_$][A-Za-z0-9_$]*\.fabr\b", app)]
    legacy += [m_.group(0) for m_ in re.finditer(r"\b[A-Za-z_$][A-Za-z0-9_$]*\.fab\b(?!r)", app)]
    legacy = [x for x in legacy if x.split(".")[0] not in NOT_DATA]
    # the named legacy-input adapter is the one place the old spelling is allowed
    ad = re.search(r"function adaptLegacyInput.*?\n\}", app, re.S)
    allowed = len(re.findall(r"\.fabr\b|\.fab\b", ad.group(0))) if ad else 0
    if len(legacy) > allowed:
        fails.append(f"{len(legacy)-allowed} legacy data consumer(s) outside the adapter: "
                     f"{sorted(set(legacy))[:5]}")
