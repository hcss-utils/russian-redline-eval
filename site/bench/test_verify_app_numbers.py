...
CASES = [
    ...
    # a slate the data does not know about must fail, not be quietly skipped
    ("a model the data has never seen", r'("?k"?\s*:\s*")haiku_4_5(")', r'\g<1>haiku_9_9\g<2>'),
    # Sol's round-21 cases: a legacy consumer under any variable name, and the Method class
    # table reverted to the 298-item NTS count
    ("legacy consumer under another name",
     r"(function\(a,b\)\{return a\.)flag_rate(-b\.)flag_rate", r"\g<1>fabr\g<2>fabr"),
    ("Method NTS count reverted to 28",
     r"(Nuclear signal \(NTS\)</td>.*?)<td>18</td>", r"\g<1><td>28</td>"),
]
